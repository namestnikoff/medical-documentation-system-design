...
![BPMN: Запись на приём](diagrams/patient_appointment_booking.png)
...
- **Механизмы:**
  - Автоматическое переключение PostgreSQL на ведомую реплику при отказе мастера
...
- **Оптимизация:**
  ...
  - Connection pooling для PostgreSQL (между backend-сервисами и PostgreSQL должен стоять PgBouncer или встроенные пулы приложений)
